...
from helperFunctions.rest import get_paging, get_query, success_message, error_message, convert_rest_request, get_uids, get_update, get_recursive
    ...
    def get(self, uid=None):
        if not uid:
            ...
            if not uids:
                ...
                fitted_firmware = [self._fit_firmware(firmware) for firmware in firmware_results]
                return success_message(dict(firmware_results=fitted_firmware), self.URL, request_data=dict(uids=uids))
        else:
            with ConnectTo(FrontEndDbInterface, self.config) as connection:
